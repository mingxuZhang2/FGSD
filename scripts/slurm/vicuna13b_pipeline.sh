...
#SBATCH --output=logs/v13b_pipe_%j.out
#SBATCH --error=logs/v13b_pipe_%j.err

# Vicuna-13B-v1.3 full pipeline:
...
TARGET=models/target/vicuna-13b-v1.3
DRAFTER=models/eagle3_drafters/EAGLE3-Vicuna1.3-13B
DATA_BASE=data/hidden_states/vicuna_13b_d10
PROBE_BASE=results/probes/vicuna_13b_d10

echo "=== Vicuna-13B pipeline on $(nvidia-smi --query-gpu=name --format=csv,noheader) — $(date) ==="
mkdir -p logs
...
for P in linear_draft_hidden mlp_draft_hidden; do
    python -u scripts/compare_signals.py \
        ...
        --output results/analysis/signal_comparison_vicuna13b_${P}.json
done

echo "ALL DONE: $(date)"
echo "Next: submit final_table.sh with vicuna13b env overrides"
